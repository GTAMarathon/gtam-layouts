      ...
      if (currentPrediction.value) {
        if (
          ...
          if (
            ...
            var teamFinishTimes = currentPrediction.value.outcomes.map(
              (outcome) => {
            ...
            if (
              ...
              var teamId = Object.keys(newVal.teamFinishTimes).find(
                (key) => newVal.teamFinishTimes[key] === teamFinishTimes[0]
              );
              var winningOutcome = currentPrediction.value.outcomes.filter(
...
async function getPredictionStatus(
  ...
  try {
    ...
    if (
      ...
      nodecg.log.info(
        '[PollsAndPredictions] get the Prediction status',
        resp.body.data[0]
      );
      return resp.body.data[0].status as PredictionStatus;
    } else {
      nodecg.log.info(
        '[PollsAndPredictions] Failed to get the Prediction',
        JSON.stringify(resp)
      );
...
async function lockPrediction(predictionId: string): Promise<void> {
  try {
    ...
    nodecg.log.error(
      '[PollsAndPredictions] Failed to lock the Prediction',
      err
    );
...
async function resolvePrediction(
  ...
  try {
    ...
    nodecg.log.error(
      '[PollsAndPredictions] Failed to resolve the Prediction',
      err
    );
  } finally {
    currentPrediction.value = undefined;
...
async function cancelPrediction(predictionId: string): Promise<void> {
  try {
    ...
    nodecg.log.error(
      '[PollsAndPredictions] Failed to cancel the Prediction',
      err
    );
  } finally {
    currentPrediction.value = undefined;
