...
if (config.enable) {
  if (!config.gqlOAuth || !config.clientID) {
    nodecg.log.warn(
      'Twitch highlights will not be active due to your OAuth not being set correctly in the config.'
    );
  } else {
...
var highlightRecording = nodecg.Replicant<Boolean>('twitchHighlightRecording', {
  defaultValue: false,
});
var startTimestamp = nodecg.Replicant<Moment | null>(
  'twitchHighlightStartTimestamp',
  { defaultValue: null }
) as unknown as NodeCG.ServerReplicant<Moment | null>;
var highlightRunData = nodecg.Replicant<RunData | null>(
  'twitchHighlightRunData',
  { defaultValue: null }
) as unknown as NodeCG.ServerReplicant<RunData | null>;
var highlightProcessing = nodecg.Replicant<Boolean>(
  'twitchHighlightProcessing',
  { defaultValue: false, persistent: false }
);
var runDataActiveRun = nodecg.Replicant<RunData>(
  'runDataActiveRun',
  'nodecg-speedcontrol'
);
...
timer.on('change', (newVal, oldVal) => {
  if (
    highlightRecording.value &&
    !highlightRunData.value &&
    oldVal &&
...
function getUserId(): any {
  ...
  client
    .request<CurrentUserData>(query)
    .then((data) => {
      if (data && data.currentUser && data.currentUser.id) {
        userId = data.currentUser.id;
        nodecg.log.info(
          'Twitch highlighting is enabled for ' +
            userId +
            '/' +
        ...
        nodecg.log.warn(
          'Twitch highlights will not be active due to not being able to get the user ID.'
        );
      }
    })
    .catch((err) =>
...
export function setEndAndCreateHighlight(): void {
  ...
  if (!highlightRunData.value) {
    nodecg.log.warn(
      'Twitch highlight will not be made due to no run being done during the recording.'
    );
    return;
  }

  var endTimestamp = moment.utc();
  queryStreamInfoAndCreateTwitchHighlight(
    startTimestamp.value,
    endTimestamp,
    clone(highlightRunData.value)
  );
...
function queryStreamInfoAndCreateTwitchHighlight(
  ...
  interface StreamData {
    user: {
      stream: {
        ...
        archiveVideo: { id: number; recordedAt: string };
      };
    };
  }

  client
    .request<StreamData>(query, variables)
    .then((data) => {
      if (
        !data ||
        !data.user ||
        !data.user.stream ||
        !data.user.stream.archiveVideo ||
        !data.user.stream.archiveVideo.recordedAt ||
        !moment(data.user.stream.archiveVideo.recordedAt).isValid() ||
        !data.user.stream.archiveVideo.id ||
        !data.user.stream.game ||
        !data.user.stream.game.id ||
        !data.user.stream.title ||
        !data.user.stream.broadcastLanguage
      ) {
        throw new Error('error: ' + JSON.stringify(data));
      }

      var pastBroadcastRecordedAt = moment(
        data.user.stream.archiveVideo.recordedAt
      );
      ...
      if (pastBroadcastRecordedAt.isAfter(startTimestamp)) {
        nodecg.log.warn(
          'Twitch highlight will not be made because the last past broadcast started after the highlight recording.'
        );
        return;
      }

      var startOffsetSeconds =
        startTimestamp.diff(pastBroadcastRecordedAt, 'seconds') +
        config.offsetStart;
      if (startOffsetSeconds < 0) {
        startOffsetSeconds = 0;
      }

      var endOffsetSeconds =
        endTimestamp.diff(pastBroadcastRecordedAt, 'seconds') +
        config.offsetEnd;
    ...
    .catch((err) => {
      nodecg.log.warn(
        'Twitch highlight will not be made because some stream infos wasnt received.',
        err
      );
    });
}
...
function createHightlight(
  ...
  client
    .request<HighlightData>(query, variables)
    .then((data) => {
      if (
        data &&
        data.createVideoHighlight &&
        data.createVideoHighlight.highlight &&
        data.createVideoHighlight.highlight.id
      ) {
    ...
    .catch((err) => {
      nodecg.log.warn(
        'Twitch highlight caused an error. but usually the highlighting worked'
      );
